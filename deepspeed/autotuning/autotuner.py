...
class Autotuner:
        ...
        if best_space_records:
            for key, val in best_space_records.items():
                ...
                if key == GLOBAL_TUNING_SPACE:
                    ...
                    num_exps = val[2]
                row.extend((num_exps, val[1], val[0]['name']))
                tab.append(row)
    ...
    def tune(self):
        ...
        stage = self.user_config.get(ZERO_OPTIMIZATION,
                                     {}).get(ZERO_OPTIMIZATION_STAGE,
                                             "all")
        user_zero_stages = stage if isinstance(stage, list) else [stage]
    ...
    def tune_space(self,
        ...
        if stage > 0 and (
            fast_best_mbs <= prev_best_mbs
            or fast_best_metric_val < prev_best_metric_val
        ):
            logger.info(
